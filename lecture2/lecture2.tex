    ...
    \begin{frame}
      ...
      \begin{frame}
          ...
          \begin{itemize}
              ...
              \item Aufruf des Editors vom prompt: \matlabInput{edit dateiname}.
      ...
      \begin{frame}
          ...
          \begin{itemize}
              \item \matlabInput{plot} trägt numerische Daten gegeneinander auf (\matlabInput{doc \matlabLink{plot}}).
              \item \matlabInput{plot(x,y)} erzeugt ein Grafikfenster (\texttt{``figure''}) indem die Werte \texttt{y} gegen \texttt{x} aufgetragen sind.
              \item Beispiel: die Wurzelfunktion im Intervall [0,6] zeichnen:
              \item \matlabInput{x=0:0.5:6; y=sqrt(x);plot(x,y)}
          \end{itemize}
      ...
      \begin{frame}
          ...
          \begin{itemize}
              \item Stilparameter in Hochkommata übergeben: \matlabInput{plot(x,c,'k:x');}
              \begin{itemize}
              ...
              \end{itemize}
              \item Achsenbereiche festsetzen: \matlabInput{xlim([minx maxx]); ylim([miny maxy])}
              \item Achsenbeschriftung: \matlabInput{xlabel('x [m]');} und \matlabInput{ylabel('c [mol/L]');}.
              \item Titel hinzufügen: \matlabInput{title('Titeltext');}
              \item Legende hinzufügen: \matlabInput{legend('c');}
              \item Matlab kennt rudimentäre LaTeX-Syntax für Strings: \matlabInput{xlabel('c [mol/m\string^3]')} ergibt hoch gestellte ``3'',
              \matlabInput{title('c\_\{cons\}')} ergibt tief gestelltes ``cons''
          \end{itemize}
      \end{frame}
      ...
      \begin{frame}
          ...
          \begin{itemize}
              \item Variante 1: mehrere Parameterpärchen übergeben: \matlabInput{plot(t1,c1,'r-',t2,c2,'b:')}
              \item Variante 2: mit \matlabInput{hold on} in Hinzufügmodus wechseln (und mit \matlabInput{hold off} wieder raus):
              \begin{itemize}
                  \item \matlabInput{plot(t1,c1,'r-');}
                  \item \matlabInput{hold on;plot(t2,c2,‘b:‘);hold off}
              \end{itemize}
              \item Ein neues Grafikfenster wird mit dem Befehl \matlabInput{figure} erzeugt:
              \begin{itemize}
                  \item \matlabInput{figure(1)}, erzeugt das Grafikfenster 1
      ...
      \begin{frame}
          ...
          \begin{itemize}
              ...
              \item Vermutung: könnte in Matlab \matlabInput{erfc} heißen (Testen Sie: \matlabInput{help \matlabLink{erfc}})
          \end{itemize}
      \end{frame}
